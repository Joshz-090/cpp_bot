...
async def save_nickname(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "2️⃣ *Now*, please create your own password for login:\n"
        "Choose a password you'll remember (min 4 characters):",
        parse_mode="Markdown"
    )
    return ASK_PASSWORD
...
async def login_nickname(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"Logging in as `{nickname}`. Please enter your password:")
...
async def finish_registration(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Validates and saves the user's own password to complete registration."""
    password = update.message.text.strip()
    ...
    # Save the user's own password (not checking against global password)
    ...
    success_msg = (
        ...
        "🚀 *Main Menu Options:*\n"
        "📚 Courses - Browse course materials\n"
        "📝 Quizzes - Take timed quizzes\n"
        "📊 My Status - View your progress\n"
...
async def handle_content_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "content_pdf":
        # Get all PDF files for this week
        pdf_files = CourseService.get_content_files(course.id, week_num, 'pdf') if course else []
        ...
        if pdf_files:
            if len(pdf_files) == 1:
                ...
                pdf = pdf_files[0]
                try:
                    await query.message.reply_document(document=pdf.file_id, caption=f"📄 Week {week_num} PDF Materials")
                except:
                    await query.message.reply_text(f"📄 *Week {week_num} PDF Materials*:\n{escape_markdown(pdf.file_url or pdf.file_id)}", parse_mode="Markdown")
            else:
                # Multiple PDFs - create list with buttons
                msg_text = f"📄 *Week {week_num} PDF Materials*\n\nAvailable PDFs:\n"
                keyboard = []
                
                for i, pdf in enumerate(pdf_files, 1):
                    name = pdf.file_name or f"PDF_{i}"
                    msg_text += f"{i}. {name}\n"
                    keyboard.append([InlineKeyboardButton(f"📄 {name}", callback_data=f"pdf_file_{pdf.id}")])
                
                keyboard.append([InlineKeyboardButton("🔙 Back", callback_data="back_to_content_options")])
                reply_markup = InlineKeyboardMarkup(keyboard)
        ...
        # Get all video files for this week
        video_files = CourseService.get_content_files(course.id, week_num, 'video') if course else []
        
        if video_files:
            if len(video_files) == 1:
                # Single video - send directly
                video = video_files[0]
                await query.message.reply_text(f"🎥 *Week {week_num} Video Lesson*:\n{escape_markdown(video.file_url)}", parse_mode="Markdown")
            else:
                # Multiple videos - create list with buttons
                msg_text = f"🎥 *Week {week_num} Video Lessons*\n\nAvailable Videos:\n"
                keyboard = []
                
                for i, video in enumerate(video_files, 1):
                    name = video.file_name or f"Video_{i}"
                    msg_text += f"{i}. {name}\n"
                    keyboard.append([InlineKeyboardButton(f"🎥 {name}", callback_data=f"video_file_{video.id}")])
        ...
        if content and content.web_link:
            await query.message.reply_text(f"🌐 *Learn more on web*:\n{escape_markdown(content.web_link)}", parse_mode="Markdown")
        else:
            # Default or placeholder
            await query.message.reply_text("🌐 *Explore More*:\nClick here for recommended web learning platform: [Learn C++ Web](https://example.com)", parse_mode="Markdown")
        ...
        await query.message.edit_message_text(f"📝 *{course_name} - Week {week_num} Quizzes*\nChoose a challenge to start:", 
                                         reply_markup=reply_markup, parse_mode="Markdown")
...
async def handle_individual_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = query.data
    if data.startswith("pdf_file_"):
        file_id = int(data.split("_")[2])
        from ..services.course_service import CourseService
        pdf_file = CourseService.get_content_file_by_id(file_id)
        if pdf_file:
            try:
                await query.message.reply_document(document=pdf_file.file_id, caption=f"📄 {pdf_file.file_name}")
            except:
                await query.message.reply_text(f"📄 *PDF File*:\n{escape_markdown(pdf_file.file_url or 'File not available')}", parse_mode="Markdown")
    
    elif data.startswith("video_file_"):
        file_id = int(data.split("_")[2])
        from ..services.course_service import CourseService
        video_file = CourseService.get_content_file_by_id(file_id)
        if video_file:
            await query.message.reply_text(f"🎥 *Video Lesson*:\n{escape_markdown(video_file.file_url)}", parse_mode="Markdown")
    
    elif data == "back_to_content_options":
        week_num = context.user_data.get('selected_week')
        return await show_content_options(update, context, week_num)
...
course_conv_handler = ConversationHandler(
    ...
    states={
        ...
        CONTENT_OPTIONS: [CallbackQueryHandler(handle_content_selection, pattern="^content_|back_to_weeks"),
                         CallbackQueryHandler(handle_individual_file, pattern="^pdf_file_|^video_file_|^back_to_content_options")],
        COURSE_QUIZ_SELECT: [
            CallbackQueryHandler(std_quiz_info, pattern="^std_quiz_"),
